...
class PPO:
    ...
    def choose_action(self,observation_stack,state_stack):
        # s = s[np.newaxis,:]
        a = self.sess.run(self.sample_op,{self.x_image:observation_stack})[0]
        # print(a)
        a0=np.clip(a[0],0,1)
        a1=np.clip(a[1],-1,1)
        a=[a0,a1]
    ...
    def train(self,observation_stack,state_stack,r,a,train_step):
        ...
        if METHOD['name'] == 'kl_pen':
            ...
            for i in range(A_UPDATE_STEPS):
                _,merged,self.out=self.sess.run([self.train_op,self.merged,self.surr], {self.x_image:observation_stack,self.tfa: a, self.tfadv: adv,self.oldpi_prob:oldpi_prob,self.tfdc_r: r})
                # print(i,"o",o[0,0])
                # print(i,"m",m[0,0])
        with open('PPO_openai.txt','a') as f:             
            f.write('oldpi_prob')             
            f.write('\r\n')             
