...
from tf_pkg.scripts.A2C_PER_image import LR_A, LR_C
...
class DQN:
    def __init__(self):
        ...
        # Initialize Network
        self.output, self.output_critic = self.network()
    ...
    def network(self):
        ...
        with tf.variable_scope('network'):
            ...
            w_Critic_1 = self.weight_variable(self.first_dense)  # w_fc1_1 = ([6400,512])
            b_Critic_1 = self.bias_variable([self.first_dense[1]])  # b_fc1_1 = ([512])
            w_Critic_2 = self.weight_variable(self.second_dense_state)  # w_fc2_1 = ([512，1])
            b_Critic_2 = self.bias_variable([self.second_dense_state[1]])  # b_fc2_1 = ([1])


            w_Actor_1 = self.weight_variable(self.first_dense)  # w_fc1_1 = ([6400,512])
            b_Actor_1 = self.bias_variable([self.first_dense[1]])  # b_fc1_1 = ([512])
            w_Actor_2 = self.weight_variable(self.second_dense_state)  # w_fc2_1 = ([512，1])
            b_Actor_2 = self.bias_variable([self.second_dense_state[1]])  # b_fc2_1 = ([1])
        ...
        Actor_output1  = tf.msatmul(h_concat,w_Actor_1)+b_Actor_1
        Actor_output2  = tf.matmul(Actor_output1, w_Actor_2)+b_Actor_2

        Critic_output1  = tf.msatmul(h_concat,w_Critic_1)+b_Critic_1
    ...
    def loss_and_train(self):
        ...
        log_prob = tf.log(self.output[0, self.a])
        self.exp_v = tf.reduce_mean(log_prob * self.td_error)
        self.train_Actor = tf.train.AdamOptimizer(LR_A).minimize(-self.exp_v)


        TD_error_tf = self.r + GAMMA * self.v_ - self.v
        self.loss = tf.square(self.td_error)    # TD_error = (r+gamma*V_next) - V_eval
        self.train_Critic = tf.train.AdamOptimizer(LR_C).minimize(self.loss)
    ...
    def train(self, minibatch, w_batch, batch_index):
        ...
        v_ = self.sess.run(self.output_critic, feed_dict = {self.action_target: action_batch,
                                                                                                                        ...
                                                                                                                        self.x_sensor:state_batch,
                                                                                                                        self.w_is: w_batch})
        TD_error_batch, _ = self.sess.run([self.TD_error, self.train_Critic],
                                          {self.s: s, self.v_: v_, self.r: r})


        feed_dict = {self.s: s, self.a: a, self.td_error: td}
        _, exp_v = self.sess.run([self.train_Actor, self.exp_v], feed_dict)


        for i_batch in range(len(batch_index)):
            self.TD_list[batch_index[i_batch]] = pow((abs(TD_error_batch) + self.eps), self.alpha)
    ...
    def save_model(self):
        # ------------------------------
        save_path = self.saver.save(self.sess, 'saved_networks/' + '10_D3QN_PER_image_add_sensor_empty_world_30m' + '_' + self.date_time + "/model.ckpt")
        # ------------------------------
