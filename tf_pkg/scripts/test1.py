...
class A2C:
    ...
    def loss_and_train(self):
        with tf.variable_scope('Train'):
            ...
            self.m=self.GAMMA *  tf.reduce_sum(self.output_critic,reduction_indices=0) - self.output_critic_ 
            ...
            loss = tf.summary.scalar("TD_error", self.loss)
            exp_v = tf.summary.scalar("exp_v", self.exp_v)
    ...
    def train(self, minibatch, w_batch, batch_index,train_step):
        observation_batch      = [batch[0] for batch in minibatch]      #生成batch[0]构成的列表
        state_batch            = [batch[1] for batch in minibatch] 
        action_batch           = [batch[2] for batch in minibatch]             #action是［0，0，0，1，0，0］的形式
        ...
        TD_error_batch, _,_,merged= self.sess.run([self.TD_error, self.train_Critic,self.train_Actor,self.merged],feed_dict ={self.x_image: observation_batch,
    ...
    def Experience_Replay(self, observation, state, action, reward, next_observation, next_state, terminal):
        ...
        if self.progress == 'Exploring':
            ...
            if terminal == True:
                ...
                V_next = [reward + self.Gamma * V_batch[0,0]]

            # print("V-bach",V_batch)
            # print("V-bach[0,0]",V_batch[0,0])
            # print(np.max(V_batch))
            # print("V-next",V_next)
            # print("r",[reward])

            # V_next.append([reward + self.Gamma * np.max(V_batch)])
            ...
            #                               observation], self.x_sensor: [state], self.output_critic_:V_next})
            TD_error= self.sess.run([self.TD_error],feed_dict ={self.r: [reward], 
            ...
            self.TD_list = np.append(self.TD_list, pow(
                (abs(TD_error[0]) + self.eps), self.alpha))
            # ###################################################################################################################

    ################################################## PER ############################################################
    def prioritized_minibatch(self):
        # Update TD_error list
        TD_normalized = self.TD_list / np.linalg.norm(self.TD_list, 1)     #所有的加和
        TD_sum = np.cumsum(TD_normalized)     #按照行累加
    ...
    def main(self):
        ...
        while True:
            ...
            if self.progress == 'Training':
                ...
                # Training
                self.train(minibatch, w_batch, batch_index,self.step-self.Num_start_training)
